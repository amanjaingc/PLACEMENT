...
vector< vector<int > > ret_temp(int DTable[][3],vector< vector <int > >min_DTable)
{
   ...
   for(int k=0;k<n;k++)
    {
       ...
       int a=DTable[k][1];
       int b=DTable[k][2];
         for(int i=0;i<min_DTable.size();i++)
         {
            for(int j=0;j<min_DTable[i].size();j++)
            {
                if(a==min_DTable[i][j])
                {
                      mymap[k]+=(char)(48+i);
...
void minimize_dfa(int DTable[][3], vector< int> initial,vector< int> Final)
{
   ...
   // vector< vector<int > > DTable;//above all four are passed
    ...
    int m=0;
    vector< vector< int> > temp1 = ret_temp(DTable,temp);
}
